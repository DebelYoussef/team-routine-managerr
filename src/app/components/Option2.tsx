...
interface OptionProps {
    ...
    schedule2: string;
    reminder2: string;
    visibility2: string; // Corrected from visiblity2
  }
  

export default function Option2({ onUpdate, emailParam, frequency2, schedule2, reminder2, visibility2 }: OptionProps) {
  ...
  const [enabled, setEnabled] = useState<boolean>(emailParam);
  const [frequency, setFrequency] = useState<string>(frequency2);
  const [day, setDay] = useState<string>(""); // assuming schedule2 has day and time
  const [time, setTime] = useState<string>("");
  const [reminder, setReminder] = useState<number>(() => {
    // Default to 0 if parsing fails or if reminder2 is not in expected format
    const parsedReminder = parseInt(reminder2.slice(0,1), 10);
    return isNaN(parsedReminder) ? 0 : parsedReminder;
  });
  const [reminderUnit, setReminderUnit] = useState<string>(reminder2.slice(2,6));
  const [visibility, setVisibility] = useState<string>(visibility2);

  useEffect(() => {

    setVisibility(visibility2);
    setFrequency(frequency2);
    setEnabled(emailParam);
    onUpdate({
      email: enabled,
      frequency,
    ...
    <div className='w-full h-full text-[#4F4F4F]'>
      ...
      <div className="w-full mt-4">
        ...
        <div className="flex mb-2">
          <input type="number" value={reminder} onChange={(e) => {
  const newValue = parseInt(e.target.value, 10);
  // Optional: Check if the parsed value is a valid number
  if (!isNaN(newValue)) {
    setReminder(newValue);
  } else {
    // Handle invalid input (e.g., set a default value or ignore)
    setReminder(0); // or any default value you'd like to set
  }
}} className=' mt-2 p-2 border border-black w-[100px] outline-none rounded-xl' />
          <Select onValueChange={setReminderUnit} value={reminderUnit}>
